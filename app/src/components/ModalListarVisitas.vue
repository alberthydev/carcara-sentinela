    ...
    >
      <div class="flex items-center justify-between mb-6 shrink-0">
        <div>
          ...
          <p class="text-sm text-gray-500 mt-1">Gerencie todos os seus agendamentos ativos.</p>
        </div>
        <button
          @click="$emit('close')"
      ...
      <div class="w-full flex-1 overflow-y-auto pr-2 custom-scrollbar">
        <div
          class="grid grid-cols-[1.5fr_1fr_1fr_1fr] px-4 mb-3 text-[13px] font-bold text-[#1E0D01] border-b border-gray-100 pb-2"
        >
          <div>Veículo</div>
          <div>Data</div>
          <div>Hora</div>
        ...
        <div v-if="visitas.length === 0" class="py-8 text-center text-sm text-gray-400 font-medium">
        ...
        <div class="flex flex-col gap-2">
          <div
            ...
            :key="visita._id"
            :class="[
              'grid grid-cols-[1.5fr_1fr_1fr_1fr] px-4 py-3 rounded-xl text-[13px] items-center transition-colors',
              index % 2 === 0 ? 'bg-[#FDEEE8]/50' : 'bg-transparent hover:bg-gray-50',
            ]"
          >
            <div class="font-bold text-[#1E0D01] flex flex-col">
              <span class="text-[11px] text-[#FD7917] uppercase tracking-wider">{{
                visita.veiculo?.placa
              }}</span>
              <span>{{ visita.veiculo?.marca }} {{ visita.veiculo?.modelo }}</span>
            </div>
            <div class="text-gray-600 font-medium">{{ visita.dataVisita }}</div>
            <div class="text-gray-600 font-medium">{{ visita.horaVisita }}</div>
            <div class="flex items-center justify-end gap-2">
              <button
                @click="$emit('editar', visita)"
...
defineProps<{ isOpen: boolean; visitas: any[] }>()
